...
# File/Fichier    : dhp/util/ITrigonometry.py
...
class ITrigonometry(ITimeMachine) :
  ...
  """
  Defines some constants(a.k.a. variables that are not allowed to vary)
  parameters values used by its sub class Trigonometry or any other class
  inheriting from it. Need to use unary tuples to do so.
  """
  ...
  def __init__(self) :
    ...
    #: Doc Cycles per hour to radians per seconds conversion factor.
    #      (NOTE: Just used by M. Foreman's tidal method up to now)
    self.CYCLES_HOUR_2_RADIANS_SECONDS= ( self.TWO_PI[0]/float(self.SECONDS_PER_HOUR[0]) ,)
